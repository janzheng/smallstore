...
### 2026-04-27 — Edit mode for sm_inbox_set_note (todo done-state)

- [x] **`sm_inbox_set_note(mode: 'edit')` — SHIPPED 2026-04-27.** Closes the todo done-state gap. Body shape: `{mode: 'edit', find: string, replace: string}`. Finds first line whose trimmed content equals `find`, rewrites it to `replace` (empty `replace` deletes the line). 404 if no match or item has no `forward_note`. Identity, labels, body all preserved; stamps `note_updated_at` like the other modes. **Why this shape:** generic line-level surgery primitive — todo done-state is one usage (wrap matched line as `- [x] ...`; the existing /todos skip rule for `[x]` excludes it on next call), but typo fixes, redactions, and partial revisions all use the same primitive. No separate "done" store, no sync issues — the note is the source of truth. 10 new tests in `tests/messaging-annotation.test.ts` (23/23 file, 751/751 messaging suite). Live verified on prod (deploy `336fc939-ad7f-4e03-b655-e29122a94daf`) — used it to actually mark the rosieland todo done end-to-end (todo count went 1→0, note preserved as record of completion). MCP `sm_inbox_set_note` schema + dispatch updated. #messaging #note-edit-mode

### 2026-04-27 — Cross-newsletter notes endpoint

- [x] **`GET /inbox/:name/notes` + `sm_inbox_notes` — SHIPPED 2026-04-27.** Closes the "do we have a way to aggregate / search / get all the notes" gap from `.brief/api-access-and-notes.md § 3`. Returns every item with a non-empty `forward_note` across slugs, slim shape (id, slug, display, dates, subject, from, note). Optional `?text=` substring filter on `forward_note` only — explicitly NOT body (distinct from existing `?text=` on `/query`). Optional `?slug=` / `?since=` / `?order=newest|oldest` (default newest by received_at) / `?limit` (default 100, max 500). New `?format=markdown` groups by slug with H2 per publisher, inlines filter metadata in header. New markdown renderer `renderAllNotes` in `src/messaging/newsletter-markdown.ts`. **Note on implementation:** the filter path of `inbox.query()` doesn't honor `options.order`, so the route hydrates and sorts in memory — same O(N) tradeoff as the rest of the cross-publisher routes, bounded by the 500-item query cap. Worth a future cleanup in `inbox.query()` itself but not blocking. 14 new tests; 741/741 messaging suite green. Live verified on prod (deploy `2a67e4a8-3b21-4d79-8ae9-4c9d41bf932f`) — search `?text=sub` correctly returned only the rosieland note. #messaging #cross-newsletter-notes

### 2026-04-27 — Lock down public manifest

- [x] **`/` no longer enumerates inboxes or endpoints — SHIPPED 2026-04-27.** Public root was returning the full inbox list (`mailroom`, `biorxiv`, `podcasts`) + every endpoint pattern unauthenticated — recon roadmap for any visitor. Stripped `/` to `{name, version, status}` and `/health` to `{status}`. Moved the full manifest behind auth at new route `GET /admin/manifest` (also expanded to include the newer routes: newsletters, todos, replay, note, webhook, auto-confirm). Auth itself unchanged — all `/inbox/*`, `/admin/*`, `/api/*`, `/peers/*` routes still token-gated as before. Deploy: `b0e93ed6-a1ec-4848-adf4-f540be814bf1`. CLAUDE.md updated to reflect the manifest move. #security #public-surface
...
- [x] **Phase 2a — markdown export endpoints — SHIPPED 2026-04-27.** `?format=markdown` on all three newsletter routes returns `Content-Type: text/markdown; charset=utf-8`. Index uses relative `./<slug>.md` links (browses cleanly as a folder in Obsidian/tigerflare); per-publisher view = profile header + chronological items + notes inlined as blockquotes; notes-only view skips items without notes. Pure renderer module `src/messaging/newsletter-markdown.ts` (no I/O — Phase 2b's cron will reuse these functions directly). 16 new tests; 720/720 messaging suite. Live verified on prod (deploy `0e9178ed-10ac-483c-827b-de7d59b041ad`) — Internet Pipes (24 issues, no notes) + Rosieland (1 issue, 1 note rendered as blockquote with View item link) both render cleanly. #messaging #markdown-export #phase2a
