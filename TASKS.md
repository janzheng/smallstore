...
- [?] **Delete retired `com.smallstore.mailroom-sync.plist.disabled-2026-04-29`** — the plist's function (pull mailroom mirror from tigerflare cloud → local disk) was absorbed by `com.tigerflare.server`'s universal space-sync during the 2026-04-29 migration. It's not just misnamed; it's redundant. Currently sitting as `.disabled-2026-04-29` to preserve history; safe to delete outright once we're confident the new wiring stays. See `.journal/2026-04-29-migration-cleanup-and-mirror-handoff.md` for context. #ops #migration-cleanup
