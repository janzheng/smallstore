...
- [!] **`Inbox` keyPrefix option** — current hardcoded `_index` + `items/` constants force one-inbox-per-adapter. Add `keyPrefix?: string` to `InboxOptions`; runtime-created inboxes default to `inbox/${name}/` for auto-isolation. Backwards-compat by leaving keyPrefix omitted on existing mailroom registration. Migration plan: zero — old data stays under old keys, new data lands under prefixed keys, no rewrites needed. Until this lands, every new runtime inbox needs a deploy change to register its own dedicated adapter. ~30-45 min #inbox-keyprefix-isolation #needs-before:multi-inbox-runtime
...
- [?] **Runtime-configurable AUTO_CONFIRM_SENDERS (admin API)** — currently allowlist lives in `wrangler.toml`; adding a provider requires a redeploy. Mirror the pattern used by `mailroom_rules` (runtime-editable rules store persisted in D1): `POST /admin/auto-confirm/senders`, `GET /admin/auto-confirm/senders`, `DELETE /admin/auto-confirm/senders/:pattern`. Start from the env-var seed at boot; runtime changes override. Also unlocks per-sender rate-limit / history surfacing. Promote when a second provider edit cycle becomes annoying #messaging #mailroom-auto-confirm-runtime-config
