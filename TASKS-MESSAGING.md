...
- [ ] **IP Digest backfill** — concrete end-to-end validation. **Test scenario covered in `tests/messaging-replay-hook.test.ts`** (3-item synthetic with chronological sort verification). Real prod backfill happens after deploy: dry-run with `{filter: {fields_regex: {subject: "IP Digest|Pipes "}}}`, verify samples, then live run. Validates the system against the user's actual data. #messaging #ipdigest-backfill

10 new tests in `tests/messaging-replay-hook.test.ts` covering `fields_only` merge semantics, dry-run no-write contract, full backfill scenario, error paths (404 unknown hook, 404 unknown inbox, 400 missing field, 501 when not enabled), and the no-op-on-already-populated invariant. 663/663 messaging suite green; `deno check mod.ts` clean. Phase 3 needs build+deploy before the real IP Digest backfill can fire.
