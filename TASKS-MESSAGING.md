...
- [?] Add bioRxiv bioinformatics / genomics / etc as more inboxes — for bioRxiv, prefer Path B re-enable (peers are already in the registry shape; toggle `disabled: false`). For non-CF-gated sources (arXiv, medRxiv via API, etc.), clone the valtown val with new `FEED_URL` + `TARGET_INBOX`. #rss-biorxiv-coverage
