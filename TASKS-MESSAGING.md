...
- [x] [done 2026-04-24: Sink/SinkContext/SinkResult types + sinks.ts (inboxSink/httpSink/functionSink) + InboxRegistration.sinks[] + registerSinks/addSink + email-handler dispatches sinks independently with try/catch per sink + cf-email preserves full headers in fields.headers. 27/27 cf-email + email-handler tests green incl. 3 new sink tests] **Sink abstraction + email-handler refactor.** #messaging #mailroom-sinks
- [x] [done 2026-04-24 Wave 1 agent A: `cloudflareD1({ messaging: true })` + messaging schema migration (10 migrations, d1_migrations tracking, single-line DDL per the D1 `exec()` gotcha) + items_fts virtual table + 4 triggers (ai/ad/au_delete/au_insert) + `query({ fts: "..." })` option. 26/26 tests pass via in-mem sqlite D1-shim. Adapter does not import from messaging/ — uses local MessagingRowInput/Output structural types for plugin invariant 1] FTS5 + D1 messaging mode #messaging #mailroom-fts5
- [x] [done 2026-04-24 Wave 1 agent B: `src/messaging/sender-index.ts` with createSenderIndex(adapter, opts) → upsert/get/query/delete. Adapter-agnostic (MemoryAdapter in tests). 16/16 tests. Normalizes address lowercase+trim, list-unsubscribe https>mailto>raw extraction, spam_count on spam/quarantine labels, tag merge with bounce→bounce-source. Types kept local to module per plugin invariant 3] Sender index #messaging #mailroom-sender-index
- [x] [done 2026-04-24 Wave 1 agent C: `src/messaging/classifier.ts` pure `classify(item) → string[]` + `classifyAndMerge(item) → InboxItem`. Labels: newsletter/list/bulk/auto-reply/bounce with 4 independent bounce signals. 37/37 tests. Label naming divergence with cf-email's auto/ooo noted below] Header-based classifier #messaging #mailroom-classifier
- [x] [done 2026-04-24 Wave 2 #4 (claude): HookVerdict ('accept'|'drop'|'quarantine'|InboxItem), HookContext, PreIngestHook/PostClassifyHook/PostStoreHook types in types.ts. RegistrationHooks on InboxRegistration. addHook(name, stage, hook) on InboxRegistry. email-handler refactored into 5 stages: parse → preIngest hooks → built-in classify (opt-out via `classify: false`) → postClassify hooks → sink fan-out → postStore hooks. Quarantine verdict auto-tags 'quarantined' label. Throwing hooks caught and logged, pipeline continues. 10 new email-handler tests covering drop/quarantine/mutate/chain-order/addHook/classify-opt-out/postStore-receives-results. 19/19 email-handler tests pass] Hook interface in pipeline #messaging #mailroom-hooks
- [x] [done 2026-04-24 Wave 1 agent D: extended `InboxFilter` with `fields_regex`/`text_regex`/`headers` (present/absent/regex). Evaluator: compile-once per invocation, invalid-regex safe-skip (no throw), case-insensitive default. filter-spec parses `<field>_regex:` + `headers.<name>:` + `text_regex:` inside `match:`. 39/39 tests (24 filter + 15 filter-spec). AND-semantics when both `from_email` and `from_email_regex` appear] Regex operator in filter.ts + filter-spec #messaging #mailroom-regex
...
- [x] [done 2026-04-24 Wave 2 agent E: `src/messaging/unsubscribe.ts` with unsubscribeSender + addSenderTag. RFC 8058 one-click (POSTs `List-Unsubscribe=One-Click` form-urlencoded). mailto: falls through with ok=false + attempted_url echoed (caller wires outbox). Sender tagged unsubscribed even when URL missing or HTTP fails. sender-index extended with setRecord (verbatim write, no aggregates). POST /inbox/:name/unsubscribe HTTP route via senderIndexFor resolver (501 if absent). 12/12 tests. MCP tool deferred — sm_inbox_* family doesn't exist yet] Unsubscribe surface #messaging #mailroom-unsubscribe
...
- [x] [done 2026-04-24 Wave 2 agent F: `src/messaging/quarantine.ts` — label-based (NOT sub-inbox) for zero-new-infrastructure + stable content-addressed ids across quarantine boundary. quarantineSink(inbox, opts) factory; quarantineItem(id)/restoreItem(id) helpers using `_ingest({ force: true })` (confirmed inbox.ts already supports it). listQuarantined() convenience over Inbox.query(labels: ['quarantined']). Consumers of main view must pass exclude_labels: ['quarantined'] — documented in module header. 19/19 tests. HTTP routes + MCP tools deferred since sm_inbox_* family doesn't exist yet] Quarantine sub-inbox + restore surface #messaging #mailroom-quarantine
...
- [x] [done 2026-04-24: `GET /inbox/:name/export?format=jsonl|json&filter=<url-encoded-json>&include=body&limit=N` streams ND-JSON (Content-Type: application/x-ndjson) with body inflation from blobs adapter when `include=body`. Filter param accepts full InboxFilter shape (regex + headers + text + labels). ~140 LOC + 9 tests. Error surface: partial-export failures land as `{"_error":"..."}` last line; bad filter JSON returns 400. `raw` and `attachments` inlining parked (see follow-ups below — body covers 80% of newsletter LLM processing)] Bulk export endpoint #messaging #newsletter-export
...
Promote when relevant; none block Wave 2 except the first.

- [!] **Label naming standardization (BLOCKS classifier wiring into pipeline, #4).** cf-email channel's inline `detectAutoReply` emits `auto` / `ooo`; Wave 1's classifier emits `auto-reply`. Both labels will appear on the same item until standardized. Options: (a) rename cf-email `auto` → `auto-reply`, keep `ooo` (it's distinct — out-of-office is a *subtype* of auto-reply); (b) standardize on `auto`; (c) keep both as independent facets. Lean (a). Decide before Wave 2 #4 wires `classifyAndMerge` into postParse hook #messaging #label-naming #needs-before:mailroom-hooks
